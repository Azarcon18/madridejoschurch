        ...
        <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box">
                ...
                <div class="info-box-content">
                    ...
                    <span class="info-box-number text-right">
                    <?php 
    // Count appointments from appointment_schedules
    $appointment = $conn->query("SELECT count(id) as total FROM appointment_schedules WHERE status = 1")
        ->fetch_assoc()['total'];

    // Count wedding schedules
    $wedding = $conn->query("SELECT count(id) as total FROM wedding_schedules WHERE status = 1")
        ->fetch_assoc()['total'];

    // Count baptism schedules
    $baptism = $conn->query("SELECT count(id) as total FROM baptism_schedules WHERE status = 1")
        ->fetch_assoc()['total'];

    // Calculate total across all schedule types
    $total_schedules = $appointment + $wedding + $baptism;

    // Format and display the total
    echo number_format($total_schedules);
?>
                    </span>
                </div>
            </div>
